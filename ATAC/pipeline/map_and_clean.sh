...
echo "Found $(($(samtools idxstats "./05_clean_BAMs/${fname}_propairs.bam" | grep "$mito_chr" | cut -f 3) / 2)) mitochondrial pairs"
...
samtools idxstats "./05_clean_BAMs/${fname}_dedup.bam" | cut -f 1 | grep -v "$mito_chr" | xargs samtools view -b "./05_clean_BAMs/${fname}_dedup.bam" > "./05_clean_BAMs/${fname}_clean.bam"
